function [N0,M,Mdash,Macute,Mdash_acute,mort_current,mort_former,mort_current_acute,mort_former_acute,extra_parms_vals, phiminusvals, phiplussvals,extra_parms_nams,age_matrix,parm_names,parm_current,parm_former]= create_params_v5atabc_acute(sens,TR,pSucc,pcom)
% 10 October calculates the acute Macute and Mdashacute
% sens of test
% TR treatment rate per year
% pSucc treatment efficacy
% pcom  treatment compliance
%Tparam = sens * pcom * pSucc *TR;
...
abc_F0_F1_current = 0.6942*-log(1-0.116);
abc_F0_F1_former = 0.7244*-log(1-0.106);
abc_F1_F2_current = 0.7052* -log(1-0.085);
abc_F1_F2_former =0.6576*-log(1-0.074);
abc_piv = 0.0865;

abc_F0_F1_current = 0.6851*-log(1-0.116);
abc_F0_F1_former =  0.7305*-log(1-0.106);
abc_F1_F2_current = 0.7085* -log(1-0.085);
abc_F1_F2_former = 0.6583*-log(1-0.074);
abc_piv = 0.0865;

abc_F0_F1_current = -log(1-0.116);
abc_F0_F1_former =  -log(1-0.106);
abc_F1_F2_current =  -log(1-0.085);
abc_F1_F2_former = -log(1-0.074);
abc_piv = 0.0865;
...
parm_names = {'delta','r_AF0','w','r_SVR4DC','r_SVR4HCC','r_F0F1','r_F1F2','r_F2F3','r_F3F4','r_F4DC','r_F4HCC',...
 ...
 parm_current(2)=52/24; parm_former(2) = parm_current(2);% acute duration is 24 weeks
...
%  
 ...
 %*************************0.031 not 0.01
 % https://www1.health.gov.au/internet/publications/publishing.nsf/Content/illicit-pubs-needle-return-2-toc~illicit-pubs-needle-return-2-app~illicit-pubs-needle-return-2-app-b~illicit-pubs-needle-return-2-app-b-3
 parm_current(11) = -log(1-0.031); parm_former(11) = parm_current(11); % F4 to HCC current then former
...
%  parm_former([14 16 17 18]) = 0;
 ...
 Mdash_acute = zeros(21,21);
 Mdash_acute(1:20,1:20) = Mdash;
 
 % out of TA
 %Mdash_acute(21,1) = -parm_current(3);  
  % into S0
 %Mdash_acute(1,21)= parm_current(3);
 ...
 % out of TA
 %Macute(21,1) = -parm_current(3);  
  % into S0
 %Macute(1,21)= parm_current(3);
...
%  curr_mort_former = zeros(1,9);
 ...
 mort_current_acute= repmat(curr_mort_pwid,21,1);
 mort_former_acute=repmat(curr_mort_former,21,1);
