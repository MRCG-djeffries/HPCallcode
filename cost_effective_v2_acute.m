function cost_effective_v2_acute(percent_treated)
% 24 Spetember 2020
...
% ********************************************
% 11 October 2020
% see the acute intervention word doc
% in F:/HCV/version5
% ********************************************
...
[N0,M,Mdash,Macute,Mdash_acute,mort_current,mort_former,mort_current_acute,mort_former_acute,extra_parms_vals, phiminusvals, phiplussvals,extra_parms_nams,age_matrix,parm_names,parm_current,parm_former]= create_params_v5atabc_acute;
...
lt2=scale10*sum(X(LT2_comps_10,:));
lt1=scale10*sum(X(LT1_comps_10,:));
lt=lt1+lt2;
dc=scale10*sum(X(DC_comps_10,:));
hcc=scale10*sum(X(HCC_comps_10,:));
f4=scale10*sum(X(F4_comps_10,:));
f3=scale10*sum(X(F3_comps_10,:));
f2=scale10*sum(X(F2_comps_10,:));
f1=scale10*sum(X(F1_comps_10,:));
f0=scale10*sum(X(F0_comps_10,:));
a0=scale10*sum(X(A_comps_10,:));
...
lt2=age_scale10*sum(Apwid.*X(LT2_comps_10,:));
lt1=age_scale10*sum(Apwid.*X(LT1_comps_10,:));
lt=lt1+lt2;
dc=age_scale10*sum(Apwid.*X(DC_comps_10,:));
hcc=age_scale10*sum(Apwid.*X(HCC_comps_10,:));
f4=age_scale10*sum(Apwid.*X(F4_comps_10,:));
f3=age_scale10*sum(Apwid.*X(F3_comps_10,:));
f2=age_scale10*sum(Apwid.*X(F2_comps_10,:));
f1=age_scale10*sum(Apwid.*X(F1_comps_10,:));
f0=age_scale10*sum(Apwid.*X(F0_comps_10,:));
a0=age_scale10*sum(Apwid.*X(A_comps_10,:));
current_pwid_baseline = lt' + hcc' + dc' + f4' + f3' + f2' + f1' + f0' + a0';
...
[ltb,dcb,hccb,f4b,f3b,f2b,f1b,f0b,s4b,s3b,s2b,s1b,s0b,t4b,t3b,t2b,t1b,t0b,a0b...
    ]=inallcomps(age_scale00,age_scale10,age_weights,age_weights_former,Xbase,...
    LT1_comps_00,LT1_comps_10,LT1_comps_01,LT1_comps_11,...
    LT2_comps_00,LT2_comps_10,LT2_comps_01,LT2_comps_11,...
    DC_comps_00,DC_comps_10,DC_comps_01,DC_comps_11,...
    HCC_comps_00,HCC_comps_10,HCC_comps_01,HCC_comps_11,...
...
y1=67;  % was 74 y1-67 + 1 is the number of years of intervention starting in 2016
...
N00=zeros(756,1);
N00(1)=560;N00(379)=435.6;N00(385)=4.4;%4.36;
treatprob=0;%10000;
% Macute(21,:)=0;
% Macute(:,21)=0;
% Mdash_acute(21,:)=0;
...
tic
[t1,x1t] = ode15s(@odeeq_v5T_2p_scaled_death_acute,0:81,N00,[],Macute,Mdash_acute,mort_current_acute,mort_former_acute,extra_parms_vals,phiminusvals, phiplussvals,age_matrix,...
    age_scale00,age_scale10,treatprob,pcom,alpha,67,y1,age_weights',age_weights_former',death_val,...
    percent_treated); % note 2015 marker is 65 years
toc
XT=x1t';
...
nlen=size(XT,2);
% scaled version
Apwid=repmat(age_weights,1,nlen);
Aformer=repmat(age_weights_former,1,nlen);
lt2=age_scale10*sum(Apwid.*X(LT2_comps_10,:))+...
    age_scale10*sum(Apwid.*X(LT2_comps_11,:));
lt1=age_scale10*sum(Apwid.*X(LT1_comps_10,:))+...
    age_scale10*sum(Apwid.*X(LT1_comps_11,:));
lt=lt1+lt2;
dc=age_scale10*sum(Apwid.*X(DC_comps_10,:))+...
   age_scale10*sum(Apwid.*X(DC_comps_11,:));
hcc=age_scale10*sum(Apwid.*X(HCC_comps_10,:))+...
    age_scale10*sum(Apwid.*X(HCC_comps_11,:));
f4=age_scale10*sum(Apwid.*X(F4_comps_10,:))+...
    age_scale10*sum(Apwid.*X(F4_comps_11,:));
f3=age_scale10*sum(Apwid.*X(F3_comps_10,:))+...
   age_scale10*sum(Apwid.*X(F3_comps_11,:));
f2=age_scale10*sum(Apwid.*X(F2_comps_10,:))+...
   age_scale10*sum(Apwid.*X(F2_comps_11,:));
f1=age_scale10*sum(Apwid.*X(F1_comps_10,:))+...
   age_scale10*sum(Apwid.*X(F1_comps_11,:));
f0=age_scale10*sum(Apwid.*X(F0_comps_10,:))+...
   age_scale10*sum(Apwid.*X(F0_comps_11,:));
a0=age_scale10*sum(Apwid.*X(A_comps_10,:))+...
   age_scale10*sum(Apwid.*X(A_comps_11,:));
...
figure;plot(t1,current_pwid_baseline,'b-',t1,lt' + hcc' + dc' + f4' + f3' + f2' + f1' + f0' + a0','r-','linewidth',2)
xticks(0:1:81)
set(gca,'xticklabel',1950:1:2031,'xticklabelrotation',90)
grid()
legend({'baseline','intervention'},'location','best')
title('Total number of HCV infected current PWID ')
set(gca,'xlim',[67 81])
yy=get(gca,'ytick')
for i = 1 : length(yy)
    yf{i} = formaty(yy(i) ,'###,###')
end
set(gca,'yticklabel',yf)
current_pwid_intervention=lt' + hcc' + dc' + f4' + f3' + f2' + f1' + f0' + a0';
% total number of cases averted
diffy=current_pwid_baseline-current_pwid_intervention;

total_averted=calc_qalyv2(diffy(time2016:end),2016,2031,1,1,0); % 2000 to 2030 in steps of 1 month

% total number of treatments
Xtest=age_scale10*sum(Apwid.*X(TA_comps_10,:));
total_tests=treatprob*calc_qalyv2(Xtest(time2016:end),2016,2031,1,1,0); % 2000 to 2030 in steps of 1 month
...
[lt,dc,hcc,f4,f3,f2,f1,f0,s4,s3,s2,s1,s0,t4,t3,t2,t1,t0,a0...
    ...
    T0_comps_00,T0_comps_10,T0_comps_01,T0_comps_11,...
    A_comps_00,A_comps_10,A_comps_01,A_comps_11);
ta=age_scale00*sum(Aformer.*XT(TA_comps_00,:))+age_scale10*sum(Apwid.*XT(TA_comps_10,:))+...
    age_scale00*sum(Aformer.*XT(TA_comps_01,:))+age_scale10*sum(Apwid.*XT(TA_comps_11,:));


figure;plot(timey,(ltb+dcb+hccb+f4b+f3b+f2b+f1b+f0b+s4b+s3b+s2b+s1b+s0b+t4b+t3b+t2b+t1b+t0b+a0b))
hold on;
plot(timey,(lt+dc+hcc+f4+f3+f2+f1+f0+s4+s3+s2+s1+s0+t4+t3+t2+t1+t0+a0+ta),'r-')
xticks(0:5:81)
set(gca,'xticklabel',1950:5:2030,'xticklabelrotation',90)
grid()
...
set(gca,'xticklabel',1950:5:2030,'xticklabelrotation',90)
grid()
title('Number of acute PWID')
[~,Ttot_liver_deaths_scaled,XTdeaths]=fatalQALYS_acute(age_scale00,age_scale10,age_weights,age_weights_former,XT,time2016,testcompvec10,testcompvec00,death_vec);
[yinci_PWID,incy]=plotintervention(XT,delta,rate,age_weights,age_weights_former,age_scale00,age_scale10,time2016,0,...
    A_comps_00,A_comps_10,A_comps_01,A_comps_11,XTdeaths);
%number of people treated
treatvec=zeros(length(t1),1);
pwid_weights=age_weights;
% ntreatacute=treatprob;
% if ntreatacute>1
% for i = 1 : length(t1)
% 
%     if i>=y1
...
%     end
%     A_pwid = 384:21:567;
%     Topwider=age_scale10*sum(pwid_weights.*X(A_pwid,i));
%     treatvec(i) = min(treaty,Topwider);
% end
% else
%     for i = 1 : length(t1)
%     if i>=y1
...
%     A_pwid = 384:21:567;
%     Topwider=age_scale10*sum(pwid_weights.*X(A_pwid,i));
%     treatvec(i) = treaty*sum(Topwider);
% end
% end
 for i = 1 : length(t1)
     if i>=y1
         treaty = percent_treated;
     else
         treaty = 0;
     end
     A_pwid = 384:21:567;
 ...
 tot_treats=sum(treatvec);
 save(['foutaplha95',num2str(100*percent_treated)],'tot_treats','incy','total_averted');
 runy=0;
 if runy==1
Tqualy_vals00=zeros(20,1); % This conatins qualy for all compartments 00 (former, not failed)
Tqualy_vals01=zeros(20,1); % This conatins qualy for all compartments 01 (former, failed)
...
function [Q_tot_liver_deaths_scaled,tot_liver_deaths_scaled,Xdeaths_scaled]=fatalQALYS_acute(age_scale00,age_scale10,age_weights,age_weights_former,X,time2016,testcompvec10,testcompvec00,death_vec)
% tot liver deaths with age weighting
% testcompvec01_failed=[192:20:360;193:20:360;194:20:360;195:20:360];
% testcompvec11_failed=[552:20:720;553:20:720;554:20:720;555:20:720];
% extra comps for acute case
testcompvec01_failed=[201:21:378;202:21:378;203:21:378;204:21:378];
testcompvec11_failed=[579:21:756;580:21:756;581:21:756;582:21:756];
Apwid=repmat(repelem(age_weights,1),1,82);
Aformer=repmat(repelem(age_weights_former,1),1,82);
death_vals10_scaled=zeros(4,1);
death_vals00_scaled=zeros(4,1);
...
function [yinci_PWID,inci]=plotintervention(XT,delta,rate,age_weights,age_weights_former,age_scale00,age_scale10,time2016,ploty,...
    A_comps_00,A_comps_10,A_comps_01,A_comps_11,XTdeaths)
Apwid=repmat(repelem(age_weights,1),1,82);
Aformer=repmat(repelem(age_weights_former,1),1,82);
T00=(1-delta)*rate*age_scale00*sum(Aformer.*XT(A_comps_00,:));
...
yinci_PWID=just_integrate(T10(time2016:82),2016,2031,1)+...
    ...
    just_integrate(T01(time2016:82),2016,2031,1)+...
    just_integrate(T11(time2016:82),2016,2031,1);
% yinci_current_PWID=just_integrate(T10(68:82),2017,2031,1)+...
%     just_integrate(T11(68:82),2017,2031,1);
yinci_current_PWID=just_integrate(T10(time2016:82),2016,2031,1)+...
    just_integrate(T11(time2016:82),2016,2031,1);
inci=100*((yinci_current_PWID/yinci_current_PWID(1)));
if ploty==1
    %figure;plot(2017:2030,100*((yinci_current_PWID/yinci_current_PWID(1))),'r-','LineWidth',2);
    figure;plot(2016:2030,100*((yinci_current_PWID/yinci_current_PWID(1))),'r-','LineWidth',2);
    grid()
...
function y = formaty(Number ,FormatPattern)
