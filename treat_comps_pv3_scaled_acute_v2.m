function [phi,phidash,moved_to_T4_former,moved_to_T4_current,total_not_T4,total_T4]=treat_comps_pv3_scaled_acute_v2(ntreatacute,y1,X,age_scale00,age_scale10,t,pcom,alpha,time2015_inODEunits,pwid_weights,former_weights,incibit)
...
A_pwid = 384:21:567;
if treaty > 1
Topwider=age_scale10*sum(pwid_weights'.*X(A_pwid));
aprop=treaty/max(1,Topwider);
else
    aprop=treaty;
end
phi=zeros(756,1);
%pwid_weightsn=pwid_weights./sum(pwid_weights);
phi(A_pwid)=(52/16.6)*min(aprop*X(A_pwid),X(A_pwid));
%phi(A_pwid)=min(aprop*X(A_pwid),X(A_pwid));
