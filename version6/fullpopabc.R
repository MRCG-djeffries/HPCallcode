fullpopabc=function(){
  # uses abc to estimate full pop model
  ...
  # these are all in extra_param_values
  NN=10000
  infection_prob=runif(NN,min=0.01,max=0.5)
  cess_rate = runif(NN,min=0.001,max=0.1)
  nu_rate = runif(NN,min=1/20,max=1/2)
  matout=matrix(nrow=2,ncol=NN)
  ...
  extra_parms_vals=c(0,0,0)# place holder, these are the calibration parameters
  ...
  for (i in 1 : NN){
      cat(paste0("iter ",i,"\n"))
      param_vals[801:803]=c(infection_prob[i],-log(1-cess_rate[i]),nu_rate[i]); # was 5.6, nu was 1/17
      dum=asode(param_vals,N,L)
      matout[,i]=dum
  }
  saveRDS(list(M=matout,I=infection_prob,C=cess_rate,NU=nu_rate),'F:/HCV/version6/abc.rds')
}
...
asode=function(param_vals,N,L){
  ...
  f1 = sum(X[L$chronic10,66])/sum(X[L$chronic00,66])
  f2 = sum(X[L$chronic10,66])/sum(X[361:540,66])
  return(c(f1,f2))
}
