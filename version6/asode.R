asode=function(){
  ...
  #N[1]=500;N[7]=160;N[361]=340;N[367]=0
  init       = N
  ...
  X=t(X[,2:721])
  dum=age_weighted(X,L$agemat_current,L$agemat_former)
  age_weights_current=dum$age_weights_current
  age_weights_former=dum$age_weights_former
  age_scale00=144000/sum(rep(age_weights_former,each=9)*X[L$chronic00,66]) # each = 9 due to 9 chronic comps
  age_scale10=40000/sum(rep(age_weights_current,each=9)*X[L$chronic10,66])
  C=getallpop(X,L,age_weights_current,age_weights_former,lt,age_scale00,age_scale10)
  p1=plot_allcomps(times,C)
  print(p1)
  p2=plot_areacomps(times,C)
  print(p2)
  age_scale00=age_scale00*C$Tformer[66]/C$Tformer
  age_scale10=age_scale10*C$Tcurrent[66]/C$Tcurrent
  C=getallpop(X,L,age_weights_current,age_weights_former,lt,age_scale00,age_scale10)
  p1_flat=plot_allcomps(times,C)
  print(p1_flat)
  p2_flat=plot_areacomps(times,C)
  print(p2_flat)
}
